(function($) {
	...
	$(document).ready(function() {
		// get the version of the loaded jQuery UI library
		jqueryUIVersion = ($.ui.version).split('.');
		
		$.each(jqueryUIVersion, function(index, value) {
			jqueryUIVersion[index] = parseInt(value);
		});
			...
			if (sp_js_vars.custom_media_loader || !sp_js_vars.is_new_media_loader) {
				...
				if (typeof wp != 'undefined' && wp.media && wp.media.editor) {
					wp.media.editor.send.attachment = function(props, attachment) {
						...
						if (sp_js_vars.rewrite_multisite_paths) {
							var image = url.split('/files/');
		
							var domain = url.substring(7).split('/')[0];
							
							if (image[1])
								url = 'http://' + domain + sp_js_vars.current_site_path + 'wp-content/blogs.dir/' + sp_js_vars.blog_id + '/files/' + image[1];
						}
						
						// add the image url into the 'Path' field
						button.parents('.info-input').find('.path').val(url);			
		...
		$('.slider-pro .display-all-options .group').each(function() {
			var group = $(this),
				groupHeader = group.find('.group-header'),
				groupState = group.find('.group-state'),
				groupFields = group.find('.group-fields');


			if (groupState.val() == 'closed')
				groupFields.hide();


			groupHeader.click(function(event, action) {
				if ((groupState.val() == 'closed' && typeof action == 'undefined') || action == 'show') {
					groupState.val('opened');
					groupHeader.removeClass('closed');
					groupFields.slideDown();
				} else if ((groupState.val() == 'opened' && typeof action == 'undefined') || action == 'hide') {
					groupState.val('closed');
					groupHeader.addClass('closed');
					groupFields.slideUp();
				}
			});
		});
		...
		$('.slider-pro .display-all-options .show-hide-groups').each(function() {
			var showAll = $(this).find('.show-all-groups'),
				hideAll = $(this).find('.hide-all-groups'),
				groups = $(this).siblings('.group');


			showAll.click(function() {
				groups.each(function() {
					$(this).find('.group-header').trigger('click', 'show');
				});
			});
			...
			hideAll.click(function() {
				groups.each(function() {
					$(this).find('.group-header').trigger('click', 'hide');
				});
			});
		});


		$('.slider-pro .getting-started-close').click(function(event) {
			event.preventDefault();

			$('.slider-pro .getting-started').hide();
	...
	function prepareSlide(slide) {
		...
		// create the tabs
		if (jqueryUIVersion[0] == 1 && jqueryUIVersion[1] < 9) {
			...
			slide.find('.slide-tabs').tabs({
				activate: function(event, ui) {
					slide.find('.selected-tab').val(ui.newPanel.index() - 1);
				},
				
				create: function() {
					$(this).tabs('option', 'active', parseInt(slide.find('.selected-tab').val()));
				}
			});			
		}
	...
	function setPostsDataSelectors(target) {
		...
		target.find('select.post-taxonomies-multiselect').each(function() {
			...
			postTaxonomiesSelectBox.multiselect({
										...
										selectedList: 2,
										open: function() {
											$(this).multiselect('widget').find('.ui-multiselect-optgroup-label ').click(function(event) {
												return false;
											})
										},
										click: function(event, ui) {
											if ($(this).multiselect('widget').find('input:checked').length <= 5) {
												...
												var dialogBox = $('<div><div class="warning-dialog-icon"></div><p>' + sp_js_vars.taxonomy_terms_limit + '</p></div>');
