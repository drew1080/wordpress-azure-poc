<div class="wrap slider-pro">
    ...
    <form action="<?php echo $action == 'edit' ? admin_url('admin.php?page=slider_pro&action=update') : admin_url('admin.php?page=slider_pro&action=create'); ?>" method="post">
        ...
        <div class="metabox-holder has-right-sidebar">
            ...
            <div class="inner-sidebar meta-box-sortables ui-sortable<?php echo get_option('slider_pro_display_all_slider_options') ? ' display-all-options' : '';?>">
				...
				<div class="postbox action">
					<div class="inside">
						...
						<input type="hidden" class="panel-state" name="panels_state[publish]" 
							   value="<?php echo sliderpro_get_panel_state($slider, 'publish'); ?>"/>
				...
				<div class="postbox">
					...
					<div class="inside">
						<input type="hidden" class="panel-state" name="panels_state[general]" 
							   value="<?php echo sliderpro_get_panel_state($slider, 'general'); ?>"/>
						...
						<fieldset>
							<legend><?php _e('Image Align & Scale', 'slider_pro'); ?></legend>
						...
						<fieldset>
							<legend><?php _e('Other', 'slider_pro'); ?></legend>
							
							<label title="disable_right_click"><?php _e('Disable Right Click', 'slider_pro'); ?></label>
							<input name="slider_settings[disable_right_click]" type="hidden" value="0"/>
							<input name="slider_settings[disable_right_click]" type="checkbox" value="1" 
								   <?php echo sliderpro_get_setting($slider_settings, 'disable_right_click') == true ? 'checked="checked"' : ''; ?>/>
						</fieldset>
					</div>
				</div> 
				...
				foreach ($sidebar_panel_settings as $category_name => $category) {
					...
					if (get_option('slider_pro_display_all_slider_options'))
						echo SP_IND_1 . '<div class="show-hide-groups"><span class="show-all-groups">Show All</span> | <span class="hide-all-groups">Hide All</span></div>' . PHP_EOL;


					echo SP_IND_2 . '<input type="hidden" class="panel-state" name="panels_state[' . $category_name . ']" 
											value="' . sliderpro_get_panel_state($slider, $category_name) . '"/>' . PHP_EOL;
					...
					if (get_option('slider_pro_display_all_slider_options')) {
						...
						foreach ($sidebar_panel_settings[$category_name] as $key => $value) {
							...
							echo SP_IND_3 . '<div class="group">' . PHP_EOL;

							if (count($sidebar_panel_settings[$category_name]) > 1) {
								$group_id = $category_name . '-' . str_replace(' ', '', $key);
								$state = sliderpro_get_panel_state($slider, $group_id, true);
								
								echo SP_IND_3 . '<div class="group-header ' . $state . '">'. $key . '</div>' . PHP_EOL;

								echo SP_IND_3 . '<input type="hidden" class="group-state" name="panels_state[' . $group_id . ']" 
												 		value="' . $state . '"/>' . PHP_EOL;
							}
							
							echo SP_IND_3 . '<div class="group-fields">' . PHP_EOL;						
							echo SP_IND_3 . '<table class="settings-container">' . PHP_EOL;

							foreach ($sidebar_panel_settings[$category_name][$key] as $setting) {
								$value = sliderpro_get_setting($slider_settings, $setting);

								echo SP_IND_4 . sliderpro_create_slider_setting_field($setting, $value) . PHP_EOL;							
							}

							echo SP_IND_3 . '</table>' . PHP_EOL;		
							echo SP_IND_3 . '</div>' . PHP_EOL;
							echo SP_IND_3 . '</div>' . PHP_EOL;
						}
				...
				<div class="postbox">
					...
					<div class="inside">
						<input type="hidden" class="panel-state" name="panels_state[slides_order]" 
							   value="<?php echo sliderpro_get_panel_state($slider, 'slides_order'); ?>"/>
				...
				<div class="postbox">
					...
					<div class="inside">
						<input type="hidden" class="panel-state" name="panels_state[custom_js_css]" 
							   value="<?php echo sliderpro_get_panel_state($slider, 'custom_js_css'); ?>"/>
